...
function New-RBFE-stg1-VMs {
param(
[Parameter(Mandatory)][string]$CsvPath,
[Parameter(Mandatory)][ValidateRange(1,4094)][int]$VlanTag,
[Parameter(Mandatory)][pscredential]$Credential,
[ValidateSet('ThreadJob','Job')][string]$Completion = 'ThreadJob'
)
# Import usernames from CSV
if (-not (Test-Path $CsvPath)) { throw "CSV file not found: $CsvPath" }
$userList = Import-Csv -Path $CsvPath
if (-not ($userList | Get-Member -Name 'UserName')) { throw "CSV must contain 'UserName' column" }
$jobs = @()
foreach ($user in $userList) {
    $name = $user.UserName
    if (-not $name -or $name -notmatch '^[\w\.-]+$') { Write-Warning "Skipping invalid username: $name"; continue }
    $jobs += New-PveVmFromTemplate -PveHost pve.tdm.local -UserName $name -VlanTag $VlanTag -Storage StudentVMS -TemplateId 301 -Credential $Credential -SkipCertificateCheck -Completion $Completion
    $jobs += New-PveVmFromTemplate -PveHost pve.tdm.local -UserName $name -VlanTag $VlanTag -Storage StudentVMS -TemplateId 302 -Credential $Credential -SkipCertificateCheck -Completion $Completion
    $jobs += New-PveVmFromTemplate -PveHost pve.tdm.local -UserName $name -VlanTag $VlanTag -NicCount 3 -Storage StudentVMS -TemplateId 303 -Credential $Credential -SkipCertificateCheck -Completion $Completion
    $jobs += New-PveVmFromTemplate -PveHost pve.tdm.local -UserName $name -VlanTag $VlanTag -NicCount 3 -Storage StudentVMS -TemplateId 304 -Credential $Credential -SkipCertificateCheck -Completion $Completion
    $jobs += New-PveVmFromTemplate -PveHost pve.tdm.local -UserName $name -VlanTag $VlanTag -NicCount 3 -Storage StudentVMS -TemplateId 305 -Credential $Credential -SkipCertificateCheck -Completion $Completion
}
# Wait for all jobs to complete
Wait-Job $jobs
$results = $jobs | Receive-Job -AutoRemoveJob -Wait
$summary = $results | Select-Object Name, VMID, Node, Result, ACLUser
Write-Host "Summary of VM creation:"
$summary | Format-Table -AutoSize
return $results
}

Export-ModuleMember -Function New-MS203VMs, New-PveVmFromTemplate, Wait-PveTas
