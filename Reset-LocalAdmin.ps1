...
$Credential = [System.Management.Automation.PSCredential]::new("administrator",(ConvertTo-SecureString "!G0LLum!" -AsPlainText -Force))
...
foreach($vm in $vms){    
    if($vm.Name -like "*CS*") {
        ...
        Invoke-Command $vmIP -Credential $Credential -ScriptBlock {Set-LocalUser "Administrator" -Password (ConvertTo-SecureString "Pa55w.rd" -AsPlainText -Force)}
    }
}